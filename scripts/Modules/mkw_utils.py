...
def get_finish_line_coordinate():
    ...
    pointA = vec3(memory.read_f32(kmp_ref+0x4C+offset+0x8+0x0),
                  0, memory.read_f32(kmp_ref+0x4C+offset+0x8+0x4))
    pointB = vec3(memory.read_f32(kmp_ref+0x4C+offset+0x8+0x8),
                  0, memory.read_f32(kmp_ref+0x4C+offset+0x8+0xC))
...
def get_time_diff_racecompletion(history):
    ...
    if curframe.prc >= curframe.grc:
        if curframe.grc > lastframe.prc:
            length = [history.get_older_frame(
                k).prc for k in range(history.size)]
            i = find_index(curframe.grc, length)
        ...
        if curframe.prc > lastframe.grc:
            length = [history.get_older_frame(
                k).grc for k in range(history.size)]
            i = find_index(curframe.prc, length)
