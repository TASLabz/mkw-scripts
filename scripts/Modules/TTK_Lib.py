...
def createRKGFile(input_data: FrameSequence, trackID: int,
    ...
    miiData = memory.read_bytes(0x90000020, 0x4C)
    
    inputHeader = [*divmod(tuplesFace, 0x100), *divmod(tuplesDI, 0x100),
                   *divmod(tuplesTrick, 0x100), 0x00, 0x00]
    
    try:
        idBytes = bytearray("RKGD", "utf-8")
        fileHeaderBytes = bytearray(fileHeader)
        inputHeaderBytes = bytearray(inputHeader)
        ...
        fileBytes = idBytes + fileHeaderBytes + miiData
        fileBytes += inputHeaderBytes + tupleBytes + padBytes
