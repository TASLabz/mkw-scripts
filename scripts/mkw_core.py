...
class RegionError(Exception):
    def __init__(self, message=f"Expected Mario Kart Wii game ID (RMCX01), got {getGameID()}"):
        super().__init__(message)

def isMKW():
    game = getGameID()

    if game == "RMCE01" or game == "RMCP01" or game == "RMCJ01" or game == "RMCK01":
        return True
    else:
        return False
...
def calc_stick_pos(center, bounding_radius, stick_x, stick_y, move_radius):
    ...
    stick_x -= 127
    stick_y -= 127
    x, y = (center[0] + (bounding_radius * (stick_x / 127)), center[1] - (bounding_radius * (stick_y / 127)))
...
def get_frame_of_input():
    id = getGameID()
    address = {"RMCE01": 0x809BF0B8, "RMCP01": 0x809C38C0, "RMCJ01": 0x809C2920, "RMCK01": 0x809B1F00}
